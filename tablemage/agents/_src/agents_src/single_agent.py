...
class SingleAgent:
    def __init__(
        ...
        self.memory_obj = Memory.from_defaults(token_limit=memory_size)
    ...
    async def chat(self, message: str) -> str:
        ...
        async def recursive_validated_chat(
            message: str,
            max_retries: int = 5,
        ) -> str:
            for i in range(max_retries):
                try:
                    with suppress_logging():
                        output = str(
                            await self._agent.run(message, memory=self.memory_obj)
                        )
                    ...
                    # rebuild the memory
                    self.memory_obj = Memory.from_defaults(
                        token_limit=self.memory_obj.token_limit
                    )
                    self.memory_obj.put(
                        ChatMessage.from_str(
                            content="Here is a summary of my dataset:\n" + summary_str,
                    ...
                    output = str(await self._agent.run(message, memory=self.memory_obj))
